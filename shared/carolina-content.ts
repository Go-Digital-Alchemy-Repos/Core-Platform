import pagesData from "./carolina-content/pages.json";
import locationsData from "./carolina-content/locations.json";
import blogData from "./carolina-content/blog.json";

export type CarolinaBlock = { type: "h2" | "h3" | "p"; text: string };

export type CarolinaPageContent = {
  slug: string;
  h1: string;
  titleTag: string;
  ...
  blocks: CarolinaBlock[];
};

export type CarolinaLocationContent = CarolinaPageContent & {
  city: string;
  state: string;
};

export type CarolinaBlogPost = CarolinaPageContent & {
...
export const carolinaPages = pagesData as Record<string, CarolinaPageContent>;
export const carolinaLocations = locationsData as CarolinaLocationContent[];
export const carolinaBlogPosts = (blogData as CarolinaBlogPost[])
  .slice()
  .sort((a, b) => (a.date < b.date ? 1 : -1));

export function getCarolinaPage(slug: string) {
  return carolinaPages[slug];
}

export function getCarolinaLocation(slug: string) {
  return carolinaLocations.find((location) => location.slug === slug);
}

export function getCarolinaBlogPost(slug: string) {
  return carolinaBlogPosts.find((post) => post.slug === slug);
}
