...
export const ECOMMERCE_FRAUD_DECISIONS = ["allow", "allow_with_alert", "manual_review", "block"] as const;
export const ECOMMERCE_FRAUD_REVIEW_STATUSES = ["not_required", "pending", "approved", "rejected"] as const;
...
export const ecommerceCustomerAddresses = pgTable("ecommerce_customer_addresses", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  customerId: varchar("customer_id").notNull().references(() => ecommerceCustomers.id, { onDelete: "cascade" }),
...
export const ecommerceOrders = pgTable("ecommerce_orders", {
  ...
  fraudSignals: jsonb("fraud_signals").$type<Array<Record<string, unknown>>>().default(sql`'[]'::jsonb`).notNull(),
...
export const ecommerceFraudEvents = pgTable("ecommerce_fraud_events", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  orderId: varchar("order_id").references(() => ecommerceOrders.id, { onDelete: "set null" }),
  ...
  matchedRules: jsonb("matched_rules").$type<Array<Record<string, unknown>>>().default(sql`'[]'::jsonb`).notNull(),
  requestSnapshot: jsonb("request_snapshot").$type<Record<string, unknown> | null>().default(sql`'{}'::jsonb`),
...
export const ecommerceFraudBlocks = pgTable("ecommerce_fraud_blocks", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
export const insertEcommerceFraudEventSchema = createInsertSchema(ecommerceFraudEvents).omit({ id: true, createdAt: true });
export const insertEcommerceFraudBlockSchema = createInsertSchema(ecommerceFraudBlocks).omit({ id: true, createdAt: true, updatedAt: true });
