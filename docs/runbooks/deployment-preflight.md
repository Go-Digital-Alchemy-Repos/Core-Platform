...
The focused tests run under existing Vitest discovery. Hosted CI runs the static source check before migration checks and the compiled check after the production build. The build emits `dist/operations/check-deployment-preflight.mjs`, which runs with plain Node and does not require a TypeScript loader in the target runtime. These static checks do not replace the separately required runtime-environment gate.
