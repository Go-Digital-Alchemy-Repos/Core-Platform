...
| 2. Better Farms adapter               | In progress | Better Farms source `f538f2b`; locked shell, theme adapter, bounded Fund a Farm registry/content, preview, runtime API fallback, authenticated public-form proxy, and route metadata                           | Complete site inventory and expand page, form, SEO, accessibility, responsive, and asset coverage incrementally                                                                           |
...
- **Evidence:** the exact `f538f2b06c2961cc7bf217c6395cedb3a5bf43cb` Better Farms implementation revision
  remains present on the pilot branch. The only later commit is an integration-document update. The current
  source passed 21 site contract and security tests, TypeScript, and a production build; Core's site-contract,
  manifest, release-manifest, and combined pilot-contract validators all passed against the paired drafts.
