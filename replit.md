...
- **Structured Logging**: Pino-based structured JSON logger with named child loggers (http, email, r2, stripe, auth, app, db, cms, metrics). Pretty-printed in dev, raw JSON in production. PII fields (email, phone, address, SSN, dateOfBirth, refereeEmail, refereePhone) are automatically redacted from all logged response bodies. Full UUID request IDs propagated via `X-Request-Id` header. Production 5xx errors return generic messages without leaking internals. All silent catch blocks now log structured warnings or have explicit justification comments.
- **Settings Caching**: `SettingsStorage` uses TTL-based in-memory cache (60s default) keyed by category and individual key. Auto-invalidated on upsert/delete. Explicit `invalidateCategory(category)` method wired into settings update route.
- **Service Client Caching**: R2, Mailgun, and Stripe clients are cached as singletons with `reset*()` functions triggered by admin settings changes. R2 `getClient()` skips DB config fetch when client is already cached.
- **Retry Utility**: `server/utils/retry.ts` provides a `retryOnce` helper applied only to truly idempotent outbound operations (R2 uploads/deletes). Not applied to email sends, Stripe, or DB mutations.
