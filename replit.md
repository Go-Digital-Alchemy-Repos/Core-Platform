...
- **Core Features**:
    ...
    - **Structured Data / JSON-LD (Phase 3)**: Modular JSON-LD generation engine at `client/src/lib/structured-data.ts`. Schema type support: Organization (all public pages via `PageLayout` + page-level injection), WebSite (all public pages), BreadcrumbList (blog posts, event detail, CMS non-home pages), Article (blog post detail), Event (event detail), VideoObject (event detail when `recordingUrl` is set), FAQPage (CMS pages with `faq` block type present in page content). JSON-LD is injected client-side via `client/src/components/shared/json-ld.tsx` using `useEffect` to append/remove `<script type="application/ld+json">` from document.head. All schema output uses real content fields only — no speculative or hidden-content markup.
    - **Technical SEO Tools (Phase 4)**: Three new admin tabs under CMS > SEO:
      1. **SEO Audit** (`/api/admin/cms/seo-audit`): Aggregates all CMS pages, blog posts, and events; computes issue signals (missing seoTitle, missing seoDescription, missing OG image, noindex, not published, no canonical, missing description, non-public visibility, missing author). Shows summary cards, items-with-issues list with direct edit links, and an issue reference guide.
      2. **Redirects Manager** (`/api/admin/cms/redirects`): Full CRUD for 301/302 redirects stored in `redirects` DB table. Active redirects are applied server-side via Express middleware before the frontend loads (non-API GET requests only). Toggle active/inactive without deleting. Supports notes.
      3. **Sitemap** (`/sitemap.xml`): Server-rendered XML sitemap endpoint. Includes all published/public CMS pages, blog posts, and events; excludes draft, noindex, and non-public content. Admin sitemap tab shows an indexed/excluded URL inventory with per-item exclusion reasons and a "View sitemap.xml" link.
