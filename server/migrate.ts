...
async function ensureEcommerceTables(migrationsFolder: string) {
  ...
  await runSqlMigrationFile(migrationsFolder, "0032_ecommerce_customer_addresses.sql");
  ...
  await runSqlMigrationFile(migrationsFolder, "0052_ecommerce_refund_notification_jobs.sql");
  await runSqlMigrationFile(migrationsFolder, "0053_ecommerce_shipment_notification_jobs.sql");
  await runSqlMigrationFile(migrationsFolder, "0054_ecommerce_order_status_notification_jobs.sql");
...
async function ensureCmsGalleryTables(migrationsFolder: string) {
  const hasCmsGalleries = await tableExists("cms_galleries");
  if (!hasCmsGalleries) {
    logger.app.info("Applying CMS galleries migration for database without gallery tables");
    await runSqlMigrationFile(migrationsFolder, "0041_cms_galleries.sql");
  }
}
