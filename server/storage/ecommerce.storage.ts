  ...
  async countFraudEventsByIdentity(params: {
    ...
    const [ipRow] = params.ip
      ? await db
          .select({ count: sql<number>`count(*)::int` })
          .from(ecommerceFraudEvents)
          .where(and(eq(ecommerceFraudEvents.ipAddress, params.ip), gte(ecommerceFraudEvents.createdAt, params.since)))
      : [{ count: 0 }];
    const [emailRow] = params.email
      ? await db
          .select({ count: sql<number>`count(*)::int` })
          .from(ecommerceFraudEvents)
          .where(and(eq(ecommerceFraudEvents.email, params.email), gte(ecommerceFraudEvents.createdAt, params.since)))
      : [{ count: 0 }];
