...
async function fetchStripeCredentials(): Promise<{
  ...
  try {
    ...
    logger.stripe.warn("Failed to load Stripe credentials from DB, falling back to env vars", {
  ...
  const stripePublishableKey =
    settings.stripe_publishable_key || process.env.STRIPE_PUBLISHABLE_KEY || "";

  if (!stripeSecretKey) {
    throw new Error(
      "Stripe secret key not configured. Set it in Admin > Settings > Integrations or via STRIPE_SECRET_KEY env var.",
    );
  }
...
export async function getStripeWebhookSecret(): Promise<string | null> {
  try {
    ...
    logger.stripe.warn("Failed to load Stripe webhook secret from DB, falling back to env var", {
      error: err instanceof Error ? err.message : String(err),
    });
  }
