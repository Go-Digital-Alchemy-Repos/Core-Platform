...
describe("ecommerce integration adapter registry", () => {
  ...
  it("exposes capability lookups and safe adapter stubs", () => {
    ...
    expect(getEcommerceIntegrationAdaptersByCapability("payment_refund").map((provider) => provider.provider))
      .toEqual(expect.arrayContaining(["stripe", "paypal", "square", "authorize_net", "braintree", "adyen", "amazon_pay", "klarna", "afterpay"]));
