...
export async function handleMembershipStripeWebhook(
  ...
  const claimToken = await storage.membership.claimWebhook("stripe", event.id, event.type);
  if (!claimToken) {
  ...
  try {
    ...
    await storage.membership.completeWebhookClaim("stripe", event.id, claimToken);
    return { received: true };
  } catch (error) {
    await storage.membership.releaseWebhookClaim("stripe", event.id, claimToken);
    throw error;
  }
}
