...
import {
  evaluateEcommerceFraud,
  recordEcommerceFraudEvent,
} from "./ecommerce-fraud.service";
...
export async function createEcommercePaymentIntent(
  ...
  const existingCustomer = typeof storage.ecommerce.getCustomerByEmail === "function"
  ...
  if (fraudEvaluation.decision === "manual_review" && !fraudEvaluation.settings.allowManualReviewOrders) {
    await recordEcommerceFraudEvent({
    ...
    throw httpError("This checkout needs review before payment. Please contact support for help with this order.", 409);
  }
  const stripe = fraudEvaluation.decision === "manual_review"
    ? null
  ...
  await recordEcommerceFraudEvent({
    eventType: fraudEvaluation.decision === "manual_review" ? "checkout_review" : "checkout_screened",
    orderId: order.id,
    customerId: customer.id,
    email: checkoutEmail,
    ipAddress: requestMeta.ip,
  ...
  if (fraudEvaluation.decision === "manual_review") {
    throw httpError("This order needs a quick review before payment. Please contact support or try again later.", 409);
  }
...
export async function recordEcommerceStripeRiskOutcome(params: {
  ...
  const updated = await storage.ecommerce.updateOrder(params.orderId, {
    ...
    stripeAddressLine1Check: charge.payment_method_details?.card?.checks?.address_line1_check ?? null,
    stripeAddressPostalCodeCheck: charge.payment_method_details?.card?.checks?.address_postal_code_check ?? null,
  });
  if (updated) {
    await recordEcommerceFraudEvent({
      ...
      matchedRules: Array.isArray(updated.fraudSignals) ? updated.fraudSignals as Array<{ code: string; label: string; score: number }> : [],
      ...
      requestSnapshot: {
        ...
        addressPostalCodeCheck: charge.payment_method_details?.card?.checks?.address_postal_code_check ?? null,
      },
    });
  }
  return updated;
