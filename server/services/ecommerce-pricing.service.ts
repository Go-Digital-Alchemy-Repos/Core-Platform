...
export interface PublicPricedCart extends Omit<
  PricedCart,
  "lines" | "coupon" | "couponValidation"
> {
...
export function effectiveVariantPrice(
  ...
  variant: EcommerceProductVariant | undefined,
  now = new Date(),
): number {
...
export function buildCouponSnapshot(
  coupon: EcommerceCoupon | null,
): Record<string, unknown> | null {
  return couponSnapshot(coupon);
}

export function toPublicCouponValidationResult(result: CouponValidationResult | undefined):
  | (Omit<CouponValidationResult, "coupon"> & {
      coupon: Record<string, unknown> | null;
    })
  | undefined {
...
export async function validateCouponForCart(input: {
  ...
  if (
    ...
    const customerUses = await storage.ecommerce.countCouponRedemptions(
      coupon.id,
      input.customerId,
      input.customerEmail,
    );
  ...
  if (coupon.customerEligibility === "specific_emails") {
    ...
    const eligibleEmails = (coupon.eligibleCustomerEmails ?? []).map((email) =>
  ...
  if (coupon.minOrderAmount != null && input.subtotalAmount < coupon.minOrderAmount) {
    return invalidCouponResult(
      ...
      coupon,
      "minimum_not_met",
      "Order minimum has not been met",
      input.subtotalAmount,
      shippingAmount,
      taxAmount,
    );
  ...
  if (!coupon.allowStacking && (input.existingDiscountAmount ?? 0) > 0) {
    return invalidCouponResult(
      ...
      "This coupon cannot be combined with another discount",
      input.subtotalAmount,
      shippingAmount,
      taxAmount,
    );
  ...
  const totalAmount = Math.max(
    0,
    input.subtotalAmount - discountAmount + shippingAmount + taxAmount,
  );
  return {
    valid: true,
    code: coupon.code,
...
function matchesShippingZone(
  zone: EcommerceShippingZone,
...
export async function getShippingRateOptions(input: {
  ...
  const matchingZoneIds = new Set(
    zones.filter((zone) => matchesShippingZone(zone, address)).map((zone) => zone.id),
  );
  return rates
    .filter(
      (rate) =>
        matchingZoneIds.has(rate.zoneId) && rateAppliesToSubtotal(rate, input.subtotalAmount),
    )
...
export async function resolveShippingRate(input: {
  ...
  if (
    !rate ||
    !activeZoneIds.has(rate.zoneId) ||
    !rateAppliesToSubtotal(rate, input.subtotalAmount)
  ) {
  ...
  const lines = await Promise.all(
    cartItems.map(async (item) => {
      ...
      if (
        ...
        (product.visibility ?? "online") !== "online"
      ) {
