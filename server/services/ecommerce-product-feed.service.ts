...
export async function isEcommerceProductFeedEnabled(): Promise<boolean> {
  ...
  return ["true", "1", "yes", "on"].includes(
    (settings.product_feed_enabled ?? "").trim().toLowerCase(),
  );
}
...
export function buildProductFeedItems(
  ...
  return products
    ...
    .map((product) => {
      const link =
        absoluteUrl(`/products/${product.urlSlug}`, siteUrl) || `/products/${product.urlSlug}`;
      return {
        id: product.sku || product.id,
        title: product.name,
        description:
          product.metaDescription || product.tagline || product.description || DEFAULT_DESCRIPTION,
        link,
