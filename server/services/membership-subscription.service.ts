...
import { insertMembershipSubscriptionSchema, type InsertMembershipSubscription, type MembershipSubscriptionStatus } from "@shared/schema";

export const manualMembershipSubscriptionSchema = insertMembershipSubscriptionSchema.extend({
  status: z.enum(["trialing", "active", "past_due", "canceled", "expired", "suspended", "manual", "incomplete"]).default("manual"),
});

export async function assignManualMembership(actorUserId: string | undefined, payload: unknown) {
  if (!payload || typeof payload !== "object") {
    throw Object.assign(new Error("Membership subscription payload is required"), { statusCode: 400 });
  }
...
export async function updateMembershipSubscriptionStatus(
  ...
  const subscription = await storage.membership.updateSubscription(subscriptionId, data as Partial<InsertMembershipSubscription>);
  if (!subscription) throw Object.assign(new Error("Membership subscription not found"), { statusCode: 404 });
  await storage.membership.createAuditEvent({
