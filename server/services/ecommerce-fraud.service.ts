...
export const ecommerceFraudSettingsSchema = z.object({
  ...
  customerDeclineMessage: z
    .string()
    .trim()
    .max(240)
    .default(
      "We could not complete this checkout. Please contact support for help with this order.",
    ),
  ...
  highRiskCountries: z
    ...
    .default([]),
  suspiciousEmailDomains: z
    .array(z.string().trim().toLowerCase())
    .default(["mailinator.com", "guerrillamail.com", "10minutemail.com"]),
...
export type EcommerceFraudSettings = Omit<
  z.infer<typeof ecommerceFraudSettingsSchema>,
  "maxMindLicenseKey"
> & {
...
function normalizeAddress(address?: CheckoutAddress | null) {
  if (!address) return "";
  return [address.address, address.line2, address.city, address.state, address.zip, address.country]
    .map((part) =>
      String(part ?? "")
        .trim()
        .toLowerCase(),
...
function chooseDecision(params: {
  ...
  if (params.score >= params.settings.riskBlockThreshold)
    return params.settings.defaultHighRiskAction;
  ...
  if (params.signals.some((signal) => signal.action === "allow_with_alert"))
...
export async function evaluateEcommerceFraud(
  input: FraudEvaluationInput,
): Promise<EcommerceFraudEvaluation> {
  ...
  for (const block of activeBlocks) {
    ...
    if (block.type === "address" && shippingAddressText.includes(block.value)) {
      signals.push({
        ...
        label: "Shipping address matched blocklist",
        score: 100,
        action: "block",
      });
    }
  ...
  if (listHas(settings.blockedEmails, email)) {
    signals.push({
      code: "settings_blocked_email",
      label: "Email is in the settings blocklist",
      score: 100,
      action: "block",
    });
  }
  if (!ipAllowed && ipMatches(input.ip, settings.blockedIpRanges)) {
    signals.push({
      code: "settings_blocked_ip",
      label: "IP address is in the settings blocklist",
      score: 100,
      action: "block",
    });
  }
  if (
    settings.blockedAddresses.some((blocked) => shippingAddressText.includes(blocked.toLowerCase()))
  ) {
    signals.push({
      code: "settings_blocked_address",
      label: "Shipping address is in the settings blocklist",
      score: 100,
      action: "block",
    });
  }
  ...
  if (!ipAllowed) {
    ...
    const attempts =
      typeof storage.ecommerce.countFraudEventsByIdentity === "function"
    ...
    if (attempts.ipAttempts >= settings.maxAttemptsPerIp) {
      signals.push({
        code: "velocity_ip",
        label: "Too many recent checkout attempts from this IP",
        score: 100,
        action: "block",
      });
    }
    if (attempts.emailAttempts >= settings.maxAttemptsPerEmail) {
      signals.push({
        code: "velocity_email",
        label: "Too many recent checkout attempts for this email",
        score: 100,
        action: "block",
      });
    }
    ...
    signals.push({
      code: "allowed_ip",
      label: "IP allowlist bypassed IP block and velocity rules",
      score: 0,
  ...
  if (
    ...
    addressesDiffer(input.shippingAddress, input.billingAddress)
  ) {
    signals.push({
  ...
  if (shippingCountry && billingCountry && shippingCountry !== billingCountry) {
    signals.push({
      ...
      label: "Billing and shipping countries differ",
      score: 25,
      action: settings.countryMismatchAction,
    });
  }
  if (shippingCountry && settings.highRiskCountries.includes(shippingCountry)) {
    signals.push({
      code: "high_risk_country",
      label: "Shipping country is on the high-risk list",
      score: 40,
      action: settings.defaultHighRiskAction,
    });
  }
  if (
    settings.suspiciousEmailDomains.includes(domain) ||
    settings.disposableEmailDomains.includes(domain)
  ) {
    signals.push({
      code: "suspicious_email_domain",
      label: "Email domain is commonly used for suspicious checkout attempts",
      score: 35,
  ...
  if (settings.maxOrderAmount > 0 && input.amount > settings.maxOrderAmount) {
    signals.push({
      ...
      label: "Order amount exceeds the configured maximum",
      score: 100,
      action: "block",
    });
  }
  if (settings.maxQuantity > 0 && input.quantity > settings.maxQuantity) {
    signals.push({
      code: "max_quantity",
      label: "Order quantity exceeds the configured maximum",
      score: 50,
      action: "manual_review",
    });
  }
  if (
    !input.customerId &&
    settings.firstOrderHighValueAmount > 0 &&
    input.amount >= settings.firstOrderHighValueAmount
  ) {
    signals.push({
  ...
  const duplicate =
    typeof storage.ecommerce.findRecentDuplicateFraudEvent === "function"
  ...
  if (duplicate) {
    signals.push({
      ...
      label: "Similar checkout attempt found recently",
      score: 30,
      action: "allow_with_alert",
    });
  }
...
export async function createEcommerceFraudBlock(
  input: unknown,
  actorId?: string | null,
): Promise<EcommerceFraudBlock> {
  const data = z
    .object({
...
export async function deleteEcommerceFraudBlock(
  id: string,
): Promise<EcommerceFraudBlock | undefined> {
