...
export async function assertPlanCanBeActive(planId: string): Promise<void> {
  ...
  if (!hasActivePrice) {
    throw Object.assign(
      new Error(
        "Paid membership plans require at least one active paid price before they can be published.",
      ),
      {
        statusCode: 400,
      },
...
export async function createMembershipPlan(payload: unknown) {
  ...
  if (existing)
    throw Object.assign(new Error("A membership plan with this slug already exists"), {
      statusCode: 409,
    });
  ...
  await Promise.all(
    prices.map((price) => storage.membership.createPrice({ ...price, planId: plan.id })),
  );
  await storage.membership.replacePlanEntitlements(
    plan.id,
    entitlements.map((entitlement) =>
      insertMembershipPlanEntitlementSchema.parse({ ...entitlement, planId: plan.id }),
    ),
  );
...
export async function updateMembershipPlan(id: string, payload: unknown) {
  ...
  if (entitlements) {
    await storage.membership.replacePlanEntitlements(
      id,
      entitlements.map((entitlement) =>
        insertMembershipPlanEntitlementSchema.parse({ ...entitlement, planId: id }),
      ),
    );
  }
