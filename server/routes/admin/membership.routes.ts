...
import {
  ...
  updateMembershipSubscriptionStatus,
} from "../../services/membership-subscription.service";
import {
...
router.put(
  "/prices/:id",
  asyncHandler(async (req, res) => {
    const price = await storage.membership.updatePrice(
      paramString(req.params.id),
      insertMembershipPlanPriceSchema.partial().parse(req.body),
    );
    if (!price) {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    res.json(
      await storage.membership.getSubscriptions({
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const payload = z
      ...
      .parse(req.body);
    res.json(
      await updateMembershipSubscriptionStatus(
        req.user?.id,
        paramString(req.params.id),
        payload.status,
        payload.note,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    res.json(
      await storage.membership.getAccessRules(
...
router.get(
  "/access-rules/:resourceType/:resourceId",
  asyncHandler(async (req, res) => {
    const rule = await storage.membership.getAccessRule(
      paramString(req.params.resourceType),
      paramString(req.params.resourceId),
    );
    res.json(rule ?? null);
  }),
);
