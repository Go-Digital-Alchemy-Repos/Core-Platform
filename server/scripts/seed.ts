...
async function seed() {
  ...
  for (let i = 0; i < therapistData.length; i++) {
    ...
    await db.insert(therapistProfiles).values({
      userId: user.id,
      title: isSarah ? null : t.title,
      bio: isSarah ? null : t.bio,
      specializations: isSarah ? [] : t.specializations,
      languages: isSarah ? [] : t.languages,
      credentials: isSarah ? null : t.credentials,
      licenseNumber: isSarah ? null : t.licenseNumber,
      practiceMode: isSarah ? "both" : t.practiceMode,
      addressLine1: isSarah ? null : t.address,
      city: isSarah ? null : t.city,
      state: isSarah ? null : t.state,
      country: isSarah ? null : t.country,
      zipCode: isSarah ? null : t.zipCode,
      latitude: isSarah ? null : t.lat,
      longitude: isSarah ? null : t.lng,
      phone: isSarah ? null : t.phone,
      website: isSarah ? null : t.website,
      acceptingClients: true,
      isApproved: isSarah ? false : true,
      isActive: true,
    });
