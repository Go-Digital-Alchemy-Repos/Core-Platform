...
export function RefundsTab() {
  ...
  const mutation = useMutation({
    mutationFn: async () => apiRequest("POST", "/api/admin/ecommerce/refunds", {
      orderId,
      amount: cents(amount),
      source,
      type: "partial",
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Undo2 className="h-5 w-5 text-rose-600" /> Refunds</CardTitle>
        <CardDescription>Record external refunds or process a gateway-backed refund when that payment provider has an operational adapter.</CardDescription>
      </CardHeader>
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="mt-1">Stripe refunds are operational through the Refunds API. PayPal, Square, Authorize.net, Braintree, Adyen, Amazon Pay, Klarna, and Afterpay are credential-ready but blocked until their provider-specific refund adapters are implemented and tested. Apple Pay and Google Pay refunds run through the underlying processor.</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={source} onValueChange={setSource}>
              ...
              <SelectContent>
                ...
                <SelectItem value="stripe">Stripe API <Badge className="ml-2">Operational</Badge></SelectItem>
