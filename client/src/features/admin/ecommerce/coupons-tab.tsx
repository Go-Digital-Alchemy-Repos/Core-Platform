...
export function CouponsTab() {
  ...
  const duplicateMutation = useMutation({
    mutationFn: async (coupon: Coupon) =>
      apiRequest("POST", `/api/admin/ecommerce/coupons/${coupon.id}/duplicate`, {
  ...
  const archiveMutation = useMutation({
    mutationFn: async (couponId: string) =>
      apiRequest("DELETE", `/api/admin/ecommerce/coupons/${couponId}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/admin/ecommerce/coupons"] }),
  });

  const toggleIdInForm = (
    key:
      | "eligibleProductIds"
      | "eligibleCategoryIds"
      | "excludedProductIds"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <TicketPercent className="h-5 w-5" /> {editingId ? "Edit coupon" : "Coupon editor"}
          </CardTitle>
          <CardDescription>
            Configure discount rules, limits, eligibility, and lifecycle dates.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={submit} className="grid gap-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select
                  value={form.type}
                  onValueChange={(type) => setForm((c) => ({ ...c, type }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setForm((c) => ({ ...c, maxRedemptions: e.target.value }))}
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(customerEligibility) =>
                    setForm((c) => ({ ...c, customerEligibility }))
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="space-y-2">
                <Label>Eligible emails</Label>
                <Input
                  value={form.eligibleCustomerEmails}
                  onChange={(e) =>
                    setForm((c) => ({ ...c, eligibleCustomerEmails: e.target.value }))
                  }
            ...
            <CouponCheckboxGroup
              ...
              onToggle={(id) => toggleIdInForm("eligibleProductIds", id)}
            />
            <CouponCheckboxGroup
              title="Eligible categories"
              items={categories.map((category) => ({ id: category.id, label: category.name }))}
              selected={form.eligibleCategoryIds}
              onToggle={(id) => toggleIdInForm("eligibleCategoryIds", id)}
            />
            <CouponCheckboxGroup
              title="Excluded products"
              items={products.map((product) => ({ id: product.id, label: product.name }))}
              selected={form.excludedProductIds}
              onToggle={(id) => toggleIdInForm("excludedProductIds", id)}
            />
            <CouponCheckboxGroup
              title="Excluded categories"
              items={categories.map((category) => ({ id: category.id, label: category.name }))}
              selected={form.excludedCategoryIds}
              onToggle={(id) => toggleIdInForm("excludedCategoryIds", id)}
            />
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="flex items-center gap-3">
                <Switch
                  checked={form.active}
                  onCheckedChange={(active) => setForm((c) => ({ ...c, active }))}
                />
                <Label>Active</Label>
              </div>
              <div className="flex items-center gap-3">
                <Switch
                  ...
                  onCheckedChange={(allowStacking) => setForm((c) => ({ ...c, allowStacking }))}
                />
                <Label>Stacking</Label>
              </div>
              <div className="flex items-center gap-3">
                <Switch
                  checked={form.applyBeforeTax}
                  onCheckedChange={(applyBeforeTax) => setForm((c) => ({ ...c, applyBeforeTax }))}
                />
                <Label>Before tax</Label>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button type="submit" disabled={saveMutation.isPending || !form.code.trim()}>
                <Save className="mr-2 h-4 w-4" /> {editingId ? "Update coupon" : "Create coupon"}
              </Button>
              {editingId ? (
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Search, filter, edit, duplicate, archive, and review coupon performance.
            </CardDescription>
          </CardHeader>
                      ...
                      <TableCell>
                        <div className="flex justify-end gap-2">
                          ...
                          <Button
                            ...
                            onClick={(event) => {
                              event.stopPropagation();
                              duplicateMutation.mutate(coupon);
                            }}
                          >
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-4">
                ...
                <Metric
                  label="Remaining"
                  value={report.remainingUses == null ? "Unlimited" : String(report.remainingUses)}
                />
              </div>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {report.recentOrders.map((order) => (
                    <TableRow key={order.orderId}>
                      ...
                      <TableCell>{formatMoney(order.discountAmount)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
