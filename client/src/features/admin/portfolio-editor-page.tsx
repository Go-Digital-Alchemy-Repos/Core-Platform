...
export default function AdminPortfolioEditorPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest(
        isNew ? "POST" : "PUT",
        isNew ? "/api/admin/portfolio/projects" : `/api/admin/portfolio/projects/${params.id}`,
        toPayload(form),
      );
    ...
    onSuccess: async (saved) => {
      await Promise.all([
        ...
        queryClient.invalidateQueries({
          predicate: (query) =>
            String(query.queryKey[0] ?? "").startsWith("/api/portfolio/projects"),
        }),
      ]);
  ...
  const updateGallery = (index: number, patch: Partial<PortfolioGalleryItem>) =>
    set(
      "gallery",
      form.gallery.map((item, i) => (i === index ? { ...item, ...patch } : item)),
    );
  const updateVideo = (index: number, patch: Partial<PortfolioVideo>) =>
    set(
      "videos",
      form.videos.map((item, i) => (i === index ? { ...item, ...patch } : item)),
    );
  const updateSection = (index: number, patch: Partial<PortfolioSection>) =>
    set(
      "sections",
      form.sections.map((item, i) => (i === index ? { ...item, ...patch } : item)),
    );
  const updateMetric = (index: number, patch: Partial<PortfolioMetric>) =>
    set(
      "metrics",
      form.metrics.map((item, i) => (i === index ? { ...item, ...patch } : item)),
    );
      ...
      <main className="flex-1 space-y-6 p-6">
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
          <section className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Core portfolio information used on archive cards and case-study headers.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select
                      value={form.industry}
                      onValueChange={(value) => set("industry", value as PortfolioForm["industry"])}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Use the CMS media library for hero and gallery images.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                ...
                <ArrayHeader
                  title="Gallery"
                  onAdd={() => set("gallery", [...form.gallery, { url: "", alt: "", caption: "" }])}
                />
                    ...
                    <Card key={index}>
                      <CardContent className="grid gap-4 p-4 md:grid-cols-[220px_1fr_auto]">
                        <CmsImageUpload
                          ...
                          onChange={(url) => updateVideo(index, { posterUrl: url })}
                          label="Poster"
                        />
                        <div className="space-y-3">
                          <Input
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Format case-study content with links, media, alignment, and rich text controls.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <ArrayHeader
                  ...
                  onAdd={() =>
                    set("sections", [...form.sections, { title: "New Section", body: "" }])
                  }
                />
          ...
          <aside className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={form.status}
                    onValueChange={(value) => set("status", value as PortfolioForm["status"])}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      set("visibility", value as PortfolioForm["visibility"])
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                    ...
                    <SelectContent>
                      {Object.entries(PORTFOLIO_VISIBILITY_LABELS).map(([value, label]) => (
                        <SelectItem key={value} value={value}>
                          {label}
                        </SelectItem>
                      ))}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <ArrayHeader
                  ...
                  onAdd={() =>
                    set("metrics", [...form.metrics, { label: "", value: "", description: "" }])
                  }
                />
            ...
            <Card>
              <CardHeader>
                <CardTitle>CTA And SEO</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-1.5">
