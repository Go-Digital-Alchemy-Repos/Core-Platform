  ...
  return (
    <PageLayout>
      <div className="container mx-auto px-4 py-8 flex flex-col gap-6 max-w-4xl">
        <Link href="/directory">
          <Button variant="ghost" data-testid="link-back-directory">
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Directory
          </Button>
        </Link>
        ...
        <Card>
          <CardHeader className="flex flex-row items-start gap-4 space-y-0">
            <Avatar className="h-16 w-16">
              ...
              <AvatarFallback data-testid="avatar-therapist" className="text-lg">
                {initials}
              </AvatarFallback>
            </Avatar>
          ...
          <CardContent className="flex flex-col gap-6">
            ...
            <section>
              <h2 className="text-lg font-semibold mb-2" data-testid="heading-contact">
                Contact Information
              </h2>
              <div className="flex flex-col gap-2 text-sm">
                {locationParts.length > 0 && (
                  <span className="inline-flex items-start gap-2" data-testid="text-address">
                    <MapPin className="h-4 w-4 mt-0.5 shrink-0 text-muted-foreground" />
                    <span>{locationParts.join(", ")}</span>
                  </span>
                )}
                {therapist.phone && (
                  <span className="inline-flex items-center gap-2" data-testid="text-phone">
                    <Phone className="h-4 w-4 shrink-0 text-muted-foreground" />
                    <a href={`tel:${therapist.phone}`} className="underline">
                      {therapist.phone}
                    </a>
                  </span>
                )}
                {therapist.user?.email && (
                  <span className="inline-flex items-center gap-2" data-testid="text-email">
                    <Mail className="h-4 w-4 shrink-0 text-muted-foreground" />
                    <a href={`mailto:${therapist.user.email}`} className="underline">
                      {therapist.user.email}
                    </a>
                  </span>
                )}
                {therapist.website && (
                  <span className="inline-flex items-center gap-2" data-testid="text-website">
            ...
            {showMap && (
              <section>
                <h2 className="text-lg font-semibold mb-2" data-testid="heading-map">
                  Location
                </h2>
                <MapView
                  therapists={[
