...
function statusColor(status: ApplicationStatus) {
  if (["active_member", "approved_pending_subscription"].includes(status))
    return "bg-green-50 dark:bg-green-950/30 border-green-500/50";
...
function statusDescription(status: ApplicationStatus) {
  const map: Record<string, string> = {
    ...
    awaiting_background_check:
      "We are preparing to initiate a background check. You will receive an email with instructions.",
    ...
    ready_for_interview:
      "Your application is ready for an interview. We'll be in touch to schedule.",
    ...
    approved_pending_subscription:
      "Congratulations! You've been approved. Activate your membership subscription to be listed in our directory.",
    active_member: "You are an active member of the platform directory.",
    denied:
      "We appreciate your interest, but we are unable to approve your application at this time. The application fee is non-refundable.",
...
function getProgressSteps(application: ApplicationStatusResponse): ProgressStep[] {
  ...
  return [
    ...
    },
    {
      ...
      detail: application.paidAt
        ? `Paid ${new Date(application.paidAt).toLocaleDateString()}`
        : undefined,
    },
    {
      ...
      status:
        bgStatus === "completed"
          ...
          : bgStatus === "in_progress"
            ? "current"
            : stepState([bgStatus], 1),
      detail:
        application.backgroundCheck?.providerFacingLabel ||
        (bgStatus === "completed"
          ? "Completed"
          : bgStatus === "in_progress"
            ? "In progress"
    ...
    },
    {
      ...
      detail:
        decStatus === "completed" ? (status === "denied" ? "Not approved" : "Approved") : undefined,
    },
  ];
}
  ...
  const items = [
    {
      ...
      description:
        "You will receive an email with instructions to complete a background check. Please complete it within 48 hours of receiving the link.",
    ...
    },
    {
      ...
      description:
        "Your 3 professional references will receive a confidential reference form via email. We recommend letting them know to expect it.",
    ...
    },
    {
      ...
      title: "Interview",
      description:
        "After your background check and references are complete, you may be invited for a brief video interview with our team.",
    ...
    },
    {
      ...
      description:
        "Our team will review your complete application and make a decision. You will be notified via email.",
    ...
    },
    {
      ...
      description:
        "Once approved, you will activate a membership subscription before your profile can go live in the directory. If your application is denied, the application fee is non-refundable.",
      highlight: false,
    },
  ];
        ...
        <div className="space-y-4">
          {items.map((item) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 mt-0.5 flex-shrink-0 ${item.highlight ? "text-blue-600" : "text-muted-foreground"}`}
                />
                <div>
                  <p
                    className={`text-sm font-medium ${item.highlight ? "text-blue-900 dark:text-blue-300" : ""}`}
                  >
                    {item.title}
                  </p>
    ...
    <TherapistLayout>
      <div className="container max-w-2xl mx-auto py-8 px-4">
        ...
        <Alert className={statusColor(status)} data-testid="banner-application-status">
          ...
          <AlertDescription>{statusDescription(status)}</AlertDescription>
        </Alert>
                  ...
                  className={`text-sm font-medium ${
                    ...
                    application.backgroundCheck.status === "completed"
                      ...
                      : application.backgroundCheck.status === "issue" ||
                          application.backgroundCheck.status === "expired"
                        ? "text-red-600"
                        : application.backgroundCheck.status === "invited"
                          ? "text-orange-600"
                          : application.backgroundCheck.status === "in_progress"
                            ? "text-blue-600"
                            : "text-muted-foreground"
                  }`}
            ...
            <Card className="mt-6">
              ...
              <CardContent>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span
                          ...
                          data-testid={`text-ref-status-${idx}`}
                        >
                          {refStatus === "completed" && (
                            <CheckCircle2 className="w-3.5 h-3.5 inline mr-1" />
                          )}
                ...
                <p className="text-xs text-muted-foreground mt-3">
                  Reference identities and responses are confidential and not shared with
                  applicants.
                </p>
              </CardContent>
            </Card>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    ${((application.amountPaid || 15000) / 100).toFixed(2)}
                  </p>
                </div>
                <div>
        ...
        {canWithdraw && (
          <div className="mt-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-3">
              If you need to withdraw your application, you can do so below. This action cannot be
              undone.
            </p>
            <Button
              variant="outline"
