...
import {
  CarolinaBlogIndexBlock,
  CarolinaCardGridBlock,
  CarolinaCtaBlock,
  CarolinaFaqBlock,
  CarolinaGalleryGridBlock,
  CarolinaHeroBlock,
  CarolinaIntroHeroBlock,
  CarolinaPageIntroBlock,
  CarolinaLocationGridBlock,
  CarolinaQuoteFormBlock,
  CarolinaRichContentBlock,
} from "@/features/carolina-public/carolina-cms-blocks";
...
const LazyTherapistMapBlock = lazy(() =>
  import("./public-dynamic-blocks").then((m) => ({
    default: m.TherapistMapBlock,
  })),
);
const LazyContactFormBlock = lazy(() =>
  import("./public-dynamic-blocks").then((m) => ({
    default: m.ContactFormBlock,
  })),
);
const LazyManagedFormEmbedBlock = lazy(() =>
  import("./public-dynamic-blocks").then((m) => ({
    default: m.ManagedFormEmbedBlock,
  })),
);
...
const LazyJoinRegistrationFormBlock = lazy(() =>
  import("./public-dynamic-blocks").then((m) => ({
    default: m.JoinRegistrationFormBlock,
  })),
);
const LazyBlogPostFeedBlock = lazy(() =>
  import("./public-dynamic-blocks").then((m) => ({
    default: m.BlogPostFeedBlock,
  })),
);
const LazyBlogFeaturedPostBlock = lazy(() =>
  import("./public-dynamic-blocks").then((m) => ({
    default: m.BlogFeaturedPostBlock,
  })),
);
const LazyStandardBlogPageBlock = lazy(() =>
  import("./public-dynamic-blocks").then((m) => ({
    default: m.StandardBlogPageBlock,
  })),
);
const LazyEventsArchiveSection = lazy(() =>
  import("@/features/public/events-page").then((m) => ({
    default: m.EventsArchiveSection,
  })),
);
...
const LazyCareerListingsSection = lazy(() =>
  import("@/features/public/careers-page").then((m) => ({
    default: m.CareerListingsSection,
  })),
);
const LazyPortfolioGridSection = lazy(() =>
  import("@/features/public/portfolio-page").then((m) => ({
    default: m.PortfolioGridSection,
  })),
);
      ...
      <div className={`rounded-2xl p-5 sm:p-8 ${variantClass}`}>
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: str(props.content) || "<p>Add callout content.</p>",
          }}
        />
        {str(props.ctaText) && (
          <div className="mt-6">
  ...
  return (
    <div>
      ...
      <div
        ...
        dangerouslySetInnerHTML={{
          __html: str(props.content) || "<p>No content.</p>",
        }}
      />
    </div>
  );
...
const RENDERERS: Record<string, React.ComponentType<{ props: Record<string, unknown> }>> = {
  ...
  "carolina-hero": CarolinaHeroBlock,
  "carolina-intro-hero": CarolinaIntroHeroBlock,
  "carolina-page-intro": CarolinaPageIntroBlock,
  "carolina-rich-content": CarolinaRichContentBlock,
  "carolina-card-grid": CarolinaCardGridBlock,
  "carolina-location-grid": CarolinaLocationGridBlock,
  "carolina-gallery-grid": CarolinaGalleryGridBlock,
  "carolina-blog-index": CarolinaBlogIndexBlock,
  "carolina-faq": CarolinaFaqBlock,
  "carolina-quote-form": CarolinaQuoteFormBlock,
  "carolina-cta": CarolinaCtaBlock,
};
