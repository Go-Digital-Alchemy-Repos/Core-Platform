        ...
        <div className={isList ? "md:flex md:items-stretch" : ""}>
          ...
          <CardContent className={isList ? "min-w-0 flex-1 space-y-4 bg-card p-5 md:p-6" : "space-y-4 p-5"}>
            ...
            {(project.summary || project.description) && (
              <p className="line-clamp-3 text-sm text-muted-foreground">{stripHtml(project.summary || project.description || "")}</p>
            )}
...
export function PortfolioGridSection({ props = {} }: { props?: Record<string, unknown> }) {
  ...
  const subheading = getSettingText(props.subheading, settings?.archiveSubheading ?? "Explore case studies, projects, and outcomes from our portfolio.");
  ...
  const showIndustryFilter = props.showIndustryFilter !== false && settings?.showIndustryFilter !== false;
  const showCategoryFilter = props.showCategoryFilter !== false && settings?.showCategoryFilter !== false;
  const showLocationFilter = props.showLocationFilter !== false && settings?.showLocationFilter !== false;
  ...
  return (
    <section className="container mx-auto space-y-8 px-4 py-10" data-testid="section-portfolio-grid">
        ...
        <section className="grid gap-3 rounded-md border bg-card p-4 md:grid-cols-[1.5fr_1fr_1fr_1fr_auto]">
          {showSearch && (
            <div className="relative">
              ...
              <Input value={q} onChange={(event) => setQ(event.target.value)} placeholder="Search projects" className="pl-9" />
            </div>
          )}
          {showIndustryFilter && (
            <Select value={industry} onValueChange={setIndustry}>
              ...
              <SelectContent>
                ...
                {(filters?.industries ?? []).map((item) => <SelectItem key={item} value={item}>{PORTFOLIO_INDUSTRY_LABELS[item]}</SelectItem>)}
              </SelectContent>
            </Select>
          )}
          {showCategoryFilter && (
      ...
      <section className={layout === "list" ? "space-y-5" : "grid gap-5 md:grid-cols-2 lg:grid-cols-3"}>
        {isLoading ? (
          Array.from({ length: layout === "list" ? 3 : 6 }).map((_, index) => <Skeleton key={index} className="h-80 w-full" />)
        ) : projects.length > 0 ? (
          projects.map((project) => <ProjectCard key={project.id} project={project} layout={layout} />)
        ) : (
          <Card className={layout === "grid" ? "md:col-span-2 lg:col-span-3" : ""}>
            <CardContent className="py-10 text-center text-muted-foreground">No portfolio projects match those filters.</CardContent>
          </Card>
        )}
      </section>
    </section>
