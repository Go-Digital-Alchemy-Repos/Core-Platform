...
function CheckoutPaymentForm({ order, email }: { order: PaymentIntentResponse; email: string }) {
  ...
  const confirmPayment = async (event: FormEvent) => {
    ...
    if (
      ...
      clearCart();
      setLocation(
        `/order-success?orderId=${encodeURIComponent(order.orderId)}&email=${encodeURIComponent(email)}&token=${encodeURIComponent(order.lookupToken)}`,
      );
      return;
    }
...
export default function CheckoutPage() {
  ...
  const eligibleSavedAddresses = savedAddresses.filter((address) =>
    allowedCountries.includes(address.country),
  );
  const regionOptions = getRegionOptions(form.country);
  const regionLabel =
    form.country === "CA" ? "Province / territory" : form.country === "US" ? "State" : "Region";
    ...
    mutationFn: async () => {
      ...
      if (normalizedAccountMode === "create_account" && !user) {
        if (form.password.length < 8)
          throw new Error("Use at least 8 characters for your account password.");
        ...
        <div className="mb-8 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Pricing is recalculated securely on the server.
            </p>
          </div>
              ...
              <CardContent>
                {!user && !isGuestOnly ? (
                  <div className="mb-6 space-y-4 rounded-lg border p-4">
                    <RadioGroup
                      ...
                      onValueChange={(accountMode) =>
                        setForm((current) => ({
                          ...current,
                          accountMode: accountMode as CheckoutAccountMode,
                        }))
                      }
                    ...
                    >
                      <label className="flex cursor-pointer items-start gap-3 rounded-lg border p-3">
                        <RadioGroupItem
                          ...
                          className="mt-1"
                          disabled={Boolean(intent) || isAccountRequired}
                        />
                        <span>
                          ...
                          <span className="text-sm text-muted-foreground">
                            Save orders and shipping details for next time.
                          </span>
                        </span>
                      </label>
                      {!isAccountRequired ? (
                        <label className="flex cursor-pointer items-start gap-3 rounded-lg border p-3">
                          ...
                          <span>
                            ...
                            <span className="block text-sm text-muted-foreground">
                              We will email secure order status links.
                            </span>
                          </span>
                        </label>
                      ) : null}
                ...
                >
                    ...
                    <div className="space-y-3 sm:col-span-2">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Choose a saved address or enter a different delivery address below.
                        </p>
                      </div>
                      <RadioGroup
                        ...
                        onValueChange={(value) => {
                          ...
                          const savedAddress = eligibleSavedAddresses.find(
                            (item) => item.id === value,
                          );
                  ...
                  <CheckoutTextField
                    ...
                    disabled={Boolean(intent)}
                    className="sm:col-span-2"
                    onChange={(couponCode) => setForm((current) => ({ ...current, couponCode }))}
                  />
                  <div className="space-y-3 sm:col-span-2">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Rates are selected from active shipping zones and calculated on the
                          server.
                        </p>
                      </div>
                      <Button
                        ...
                        disabled={
                          ...
                          !form.country.trim() ||
                          (regionOptions.length > 0 && !form.state.trim())
                        }
                    ...
                    {shippingQuoted ? (
                      ...
                      <p className="rounded-lg border p-3 text-sm text-muted-foreground">
                        Choose your shipping country and {regionLabel.toLowerCase()} to update rates
                        before payment.
                      </p>
                    )}
                  </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="border-t pt-4">
                  <div className="flex justify-between font-semibold">
                    ...
                    <span>{formatMoney(intent?.priced.subtotalAmount ?? subtotal)}</span>
                  </div>
                  ...
                  {intent ? (
                    <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                      <div className="flex justify-between">
                        <span>Discount</span>
                        <span>-{formatMoney(intent.priced.discountAmount)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Tax</span>
                        <span>{formatMoney(intent.priced.taxAmount)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Shipping</span>
                        <span>{formatMoney(intent.priced.shippingAmount)}</span>
                      </div>
                      <div className="flex justify-between border-t pt-2 text-base font-semibold text-foreground">
                        <span>Total</span>
                        <span>{formatMoney(intent.priced.totalAmount)}</span>
                      </div>
                    </div>
                  ) : (
                    <p className="mt-2 text-xs text-muted-foreground">
                      Final tax, discount, and Stripe PaymentIntent totals are calculated by the
                      server.
                    </p>
                  )}
                </div>
                ...
                {!intent ? (
                  <Button
                    ...
                    className="w-full"
                    disabled={mutation.isPending || !canCreatePayment}
                  >
                    {mutation.isPending ? (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    ) : (
